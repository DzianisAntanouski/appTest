...
export default class App extends BaseController {
  public onInit(): void {
    ...
    if (authorization)
    void FetchDataBase.checkUserToken((JSON.parse(authorization) as IAuthObject).email)
      .then((resp) => {
        (resp as IAuthObject).email == (JSON.parse(authorization) as IAuthObject).email && (resp as IAuthObject).idToken == (JSON.parse(authorization) as IAuthObject).idToken 
